...
nameT = 'direct_f/'  #####################  direct_f  matrix_f   product_f   SA_f
j = 0
while j < 4:
    if j == 0:
        nameT = 'direct_f/'
        name = 'Direct splicing'
    elif j == 1:
        nameT = 'matrix_f/'
    ...
    while counter0 < 5:
        ...
        RAPName = nameT + str(counter0) + '/' + 'RealAndPredictionA+B0.csv'
        RAPNameProb = nameT + str(counter0) + '/' + 'RealAndPredictionProbA+B0.csv'
    ...
    plt.plot(mean_P, mean_R, color=colorlist[j], ls=lslist[j],
             label=r'%s (AUPR = %0.4f)' % (name, mean_RPs),
             lw=2, alpha=0.8)
    j = j + 1
...
#     PAndR.append(pair)
#     counter = counter + 1
# storFile(PAndR, nameT + 'RNAInterPAndRA+B.csv')
...
plt.xlim([-0.05, 1.05])
plt.ylim([-0.05, 1.05])
plt.title('Precision-Recall curve of mean-AP of different fusion method')
# 画网格
plt.grid(linestyle='-')
# 画对角线
