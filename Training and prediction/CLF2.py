...
if __name__ == '__main__':
    ...
    AllMiSequence = []
    ReadMyCsv1(AllMiSequence, 'phage sequence.csv')
    miRNAEmbedding = []
    ReadMyCsv3(miRNAEmbedding, 'phage_all_Embedding64.csv')
    ...
    '''
    AllNodeBehavior = []
    ReadMyCsv1(AllNodeBehavior, 'All_phi_SDNE64.csv')

# ------第一折----
    PositiveSample_Train = []
    ReadMyCsv1(PositiveSample_Train, 'PositiveSample_Train_all_phi4.csv')
    PositiveSample_Validation = []
    ReadMyCsv1(PositiveSample_Validation, 'PositiveSample_Validation_all_phi4.csv')
    PositiveSample_Test = []
    ReadMyCsv1(PositiveSample_Test, 'PositiveSample_Test_all_phi4.csv')

    NegativeSample_Train = []
    ReadMyCsv1(NegativeSample_Train, 'NegativeSample_Train_all_phi4.csv')
    NegativeSample_Validation = []
    ReadMyCsv1(NegativeSample_Validation, 'NegativeSample_Validation_all_phi4.csv')
    NegativeSample_Test = []
    ReadMyCsv1(NegativeSample_Test, 'NegativeSample_Test_all_phi4.csv')
...
#     print('RF_clf_Score', prediction)
#     print('RF_clf_Proba', forest.predict_proba(X_test))
# 输出预测值
...
# #--------逻辑回归---------
#     LR_clf = LogisticRegression(tol=0.000001, random_state=1998, solver="newton-cg",C=14.7)#创建分类器
#     LR_clf.fit(X_train, np.array(y_train_Pre))#用训练数据拟合分类器模型
#     prediction = LR_clf.score(X_test,np.array(y_test_Pre))
#     print('LR_clf_Score', prediction)
#     print('LR_clf_Proba', LR_clf.predict_proba(X_test))
# # 输出预测值
...
# # #-----SVM分类-------
#     SVM_clf = svm.SVC(probability = True, C=0.5) #probability = True必须加上这句，否则报错
#     SVM_clf.fit(X_train, np.array(y_train_Pre))
#     prediction = SVM_clf.score(X_test, np.array(y_test_Pre))
#     print('SVM_clf_Score', prediction)
#     print('SVM_clf_Proba', SVM_clf.predict_proba(X_test))
# # # 输出预测值
...
#     print('KNN_clf_Score', prediction)
#     print('KNN_clf_Proba', KNN_clf.predict_proba(X_test))
# # # 输出预测值
...
# #AdaBoost Classifier
#     AdaB_clf = AdaBoostClassifier(learning_rate=0.3, n_estimators=100)
#     AdaB_clf.fit(X_train, np.array(y_train_Pre))
#     prediction = AdaB_clf.score(X_test, np.array(y_test_Pre))
#     print('AdaB_clf_Score', prediction)
#     print('AdaB_clf_Proba', AdaB_clf.predict_proba(X_test))
# # # 输出预测值
#     TestOutput(AdaB_clf, 'AdaB_')

# GBDT(Gradient Boosting Decision Tree) Classifier
    GBDT_clf = GradientBoostingClassifier(learning_rate=0.2, n_estimators=150)
    GBDT_clf.fit(X_train,np.array(y_train_Pre))
    prediction = GBDT_clf.score(X_test, np.array(y_test_Pre))
    print('GBDT_clf_Score', prediction)
    print('GBDT_clf_Proba', GBDT_clf.predict_proba(X_test))

    TestOutput(GBDT_clf, 'GBD_')
