...
if __name__ == '__main__':
    ...
    AllMiSequence = []
    ReadMyCsv1(AllMiSequence, 'phage_dna.csv')
    miRNAEmbedding = []
    ReadMyCsv3(miRNAEmbedding, 'phage_DNA_Embedding.csv')

    AllProtein = []
    ReadMyCsv1(AllProtein, 'phage sequence.csv')    #
    ProteinEmbedding = []
    ReadMyCsv3(ProteinEmbedding, 'phage_Embedding.csv')     #

    AllNodeBehavior1 = []
    ReadMyCsv1(AllNodeBehavior1, 'All_phage_SDNE_Behavior256.csv')

    # AllNodeBehavior2 = []
    # ReadMyCsv1(AllNodeBehavior2, 'All_HV_SDNE_Behavior1.csv')

    PositiveSample_Train = []
    ReadMyCsv1(PositiveSample_Train, 'PositiveSample_Train_pe.csv')
    PositiveSample_Validation = []
    ReadMyCsv1(PositiveSample_Validation, 'PositiveSample_Validation_pe.csv')
    PositiveSample_Test = []
    ReadMyCsv1(PositiveSample_Test, 'PositiveSample_Test_pe.csv')

    NegativeSample_Train = []
    ReadMyCsv1(NegativeSample_Train, 'NegativeSample_Train_pe.csv')
    NegativeSample_Validation = []
    ReadMyCsv1(NegativeSample_Validation, 'NegativeSample_Validation_pe.csv')
    NegativeSample_Test = []
    ReadMyCsv1(NegativeSample_Test, 'NegativeSample_Test_pe.csv')
    ...
    ProteinEmbeddingFeature = GenerateEmbeddingFeature(AllProtein, ProteinEmbedding, 256)   # manual!!!!!!!
    ...
    while CounterT < 5:
        ...
        x1 = Conv2D(8, kernel_size=(4, 64), activation='relu', name='conv1')(input1)
        x1 = MaxPooling2D(pool_size=(1, 1), name='pool1')(x1)
        x1 = Flatten()(x1)
        x1 = Dense(16, activation='relu', activity_regularizer=regularizers.l2(0.001))(x1)
        x1 = Dropout(rate=0.1)(x1)
        # ——输入2——
        input2 = Input(shape=(len(x_train_2_Attribute[0]), len(x_train_2_Attribute[0][0]), 1), name='input2')
        x2 = Conv2D(8, kernel_size=(4, 64), activation='relu', name='conv2')(input2)
        x2 = MaxPooling2D(pool_size=(1, 1), name='pool2')(x2)
        x2 = Flatten()(x2)
        x2 = Dense(8, activation='relu', activity_regularizer=regularizers.l2(0.001))(x2)
        x2 = Dropout(rate=0.1)(x2)
        # ——输入3——
        input3 = Input(shape=(len(x_train_1_Behavior1[0]),), name='input3')
        x3 = Dense(8, activation='relu', activity_regularizer=regularizers.l2(0.001))(input3)
        x3 = Dropout(rate=0.1)(x3)
        # ——输入4——
        input4 = Input(shape=(len(x_train_2_Behavior1[0]),), name='input4')
        x4 = Dense(8, activation='relu', activity_regularizer=regularizers.l2(0.001))(input4)
        ...
        hidden = Dropout(rate=0.1)(hidden)
        # hidden = Dense(256, activation='relu', name='hidden4', activity_regularizer=regularizers.l2(0.001))(hidden)
        # hidden = Dropout(rate=0.1)(hidden)
