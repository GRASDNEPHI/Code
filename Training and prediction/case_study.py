...
if __name__ == '__main__':
    ...
    AllProtein = []
    ReadMyCsv1(AllProtein, 'Maize_list_sequence.csv')    #
    ProteinEmbedding = []
    ReadMyCsv3(ProteinEmbedding, 'ProteinEmbedding.csv')     #

    AllNodeBehavior = []
    ReadMyCsv1(AllNodeBehavior, 'AllNodeBehavior1.csv')
    ...
    # NegativeSample_Test = []
    # ReadMyCsv1(NegativeSample_Test, 'test.csv')
    ...
    ReadMyCsv1(x_test_pair, 'test.csv')
    ...
    print(x_test_1_Attribute.shape)
    print(x_test_2_Attribute.shape)
    ...
    y_test_Pre = []
    for i in range(int(len(x_test_pair))):
        y_test_Pre.append(0)
    ...
    while CounterT < 1:
        ...
        x1 = Flatten()(x1)
        x1 = Dense(128, activation='relu', activity_regularizer=regularizers.l2(0.001))(x1)
        x1 = Dropout(rate=0.1)(x1)
        # ——输入2——
        input2 = Input(shape=(len(x_train_2_Attribute[0]), len(x_train_2_Attribute[0][0]), 1), name='input2')
        x2 = Conv2D(128, kernel_size=(4, 64), activation='relu', name='conv2')(input2)
        ...
        x4 = Dense(128, activation='relu', activity_regularizer=regularizers.l2(0.001))(input4)
        ...
        # ——全连接——
        hidden = Dense(128, activation='relu', name='hidden2', activity_regularizer=regularizers.l2(0.001))(flatten)
        hidden = Dropout(rate=0.1)(hidden)
        hidden = Dense(124, activation='relu', name='hidden3', activity_regularizer=regularizers.l2(0.001))(hidden)
        hidden = Dropout(rate=0.1)(hidden)
