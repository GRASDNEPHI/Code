...
if __name__ == '__main__':
    ...
    ProteinEmbedding = []
    ReadMyCsv3(ProteinEmbedding, 'phage_all_Embedding64.csv')     #

    AllNodeBehavior1 = []
    ReadMyCsv1(AllNodeBehavior1, 'ALL_phi_lap_Behavior64.csv')
    ...
    ReadMyCsv1(PositiveSample_Train, 'PositiveSample_Train_all_phi.csv')
    PositiveSample_Validation = []
    ReadMyCsv1(PositiveSample_Validation, 'PositiveSample_Validation_all_phi.csv')
    PositiveSample_Test = []
    ReadMyCsv1(PositiveSample_Test, 'PositiveSample_Test_all_phi.csv')

    NegativeSample_Train = []
    ReadMyCsv1(NegativeSample_Train, 'NegativeSample_Train_all_phi.csv')
    NegativeSample_Validation = []
    ReadMyCsv1(NegativeSample_Validation, 'NegativeSample_Validation_all_phi.csv')
    NegativeSample_Test = []
    ReadMyCsv1(NegativeSample_Test, 'NegativeSample_Test_all_phi.csv')
    ...
    while CounterT < 5:
        ...
        x2 = Conv2D(16, kernel_size=(4, 64), activation='softmax', name='conv2')(input2)
        x2 = MaxPooling2D(pool_size=(1, 1), name='pool2')(x2)
        x2 = Flatten()(x2)
        x2 = Dense(32, activation='tanh', activity_regularizer=regularizers.l2(0.001))(x2)
        x2 = Dropout(rate=0.1)(x2)
        # ——输入3——
        input3 = Input(shape=(len(x_train_1_Behavior1[0]),), name='input3')
        x3 = Dense(16, activation='tanh', activity_regularizer=regularizers.l2(0.001))(input3)
        x3 = Dropout(rate=0.1)(x3)
        # ——输入4——
        input4 = Input(shape=(len(x_train_2_Behavior1[0]),), name='input4')
        x4 = Dense(32, activation='tanh', activity_regularizer=regularizers.l2(0.001))(input4)
        ...
        while counter < len(ModelTestOutput):
            ...
            counter = counter + 1
        LabelPredictionProbName = 'lap_RealAndPredictionProbA+B' + str(CounterT) + '.csv'
        StorFile(LabelPredictionProb, LabelPredictionProbName)
        LabelPredictionName = 'lap_RealAndPredictionA+B' + str(CounterT) + '.csv'
