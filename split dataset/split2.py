...
if __name__ == '__main__':
    ...
    relationship = []
    read_csv(relationship, 'DNA RBP HOST pairs.csv')
    random.shuffle(relationship)    # random shuffle
    relationship_train = relationship[0: int(0.7 * len(relationship))]
    store_csv(relationship_train, './line0/PositiveSample_Train_all_phi4.csv')
    relationship_validation = relationship[int(0.7 * len(relationship)):int(0.8 * len(relationship))]
    store_csv(relationship_validation, './line0/PositiveSample_Validation_all_phi4.csv')
    relationship_test = relationship[int(0.8 * len(relationship)):]
    store_csv(relationship_test, './line0/PositiveSample_Test_all_phi4.csv')
    ...
    relationship_matrix.to_csv('./line0/Relationship_Matrix.csv')

    store_csv(negative_sample, './line0/NegativeSample_all_phi4.csv')
    negative_sample_train = negative_sample[0: int(0.7 * len(negative_sample))]
    store_csv(negative_sample_train, './line0/NegativeSample_Train_all_phi4.csv')
    negative_sample_validation = negative_sample[int(0.7 * len(negative_sample)):int(0.8 * len(negative_sample))]
    store_csv(negative_sample_validation, './line0/NegativeSample_Validation_all_phi4.csv')
    negative_sample_test = negative_sample[int(0.8 * len(negative_sample)):]
    store_csv(negative_sample_test, './line0/NegativeSample_Test_all_phi4.csv')
