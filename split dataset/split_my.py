...
if __name__ == '__main__':
    ...
    relationship = []
    read_csv(relationship, './case/phage pairs.csv')
    random.shuffle(relationship)    # random shuffle
    relationship_train = relationship[0: int(0.8 * len(relationship))]
    store_csv(relationship_train, './case/PositiveSample_Train.csv')
    relationship_validation = relationship[int(0.8 * len(relationship)):]
    store_csv(relationship_validation, './case/PositiveSample_Validation.csv')
    ...
    relationship_matrix.to_csv('./case/Relationship_Matrix.csv')

    store_csv(negative_sample, './case/NegativeSample.csv')
    negative_sample_train = negative_sample[0: int(0.8 * len(negative_sample))]
    store_csv(negative_sample_train, 'case/NegativeSample_Train.csv')
    negative_sample_validation = negative_sample[int(0.8 * len(negative_sample)):]
    store_csv(negative_sample_validation, './case/NegativeSample_Validation.csv')
